...
public class ExpenseSpecifications{
	...
	public static Specification<Expense> belongsTo(User user) {
        return (root, query, cb) -> cb.equal(root.get("user"), user);
    }

    public static Specification<Expense> byDateRange(LocalDate start, LocalDate end) {
        return (root, query, cb) -> cb.between(root.get("expenseDate"), start, end);
    }

    public static Specification<Expense> byMonthAndYear(int month, int year) {
        ...
        if (thirtyDaysMonth.contains(month)) {
        	...
        	return (root, query, cb) -> cb.between(root.get("expenseDate"), LocalDate.now(), LocalDate.now());
        }
